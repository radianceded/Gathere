...
- 用户提到人均预算（如"人均100以内"）时，调用 recommend_places 时传 max_cost 参数。
- 每位参与者可以有自己的出行方式（participants 里每个人可带 mode 字段），例如"小王开车来"时给小王指定 mode=driving。
- 如果用户追加其他约束（如"要有包间"），把约束加入搜索关键词。
- 如果用户想换一种场所类型，重新推荐。
